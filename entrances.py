...
def calculate_monthly_totals(counts, time_zone):
    ...
    for key, group in itertools.groupby(
        ...
        totals.append({
            'Month': f'{key}-01',
            'Total Entrances': sum([c['interval']['analytics']['entrances'] for c in group])
        })
...
def write_detailed_data_to_csv(spaces, start, end, interval, tag, time_segment_labels):
    ...
    field_names = ['Space', 'Date', 'Total Entrances']
        ...
        if interval == 'DAILY':
            for count in space['counts']:
                writer.writerow({
                    ...
                    'Total Entrances': count['interval']['analytics']['entrances'],
                })
        else:
            for total in calculate_monthly_totals(space['counts'], time_zone):
                writer.writerow({
                    'Space': space['name'],
                    'Date': total['Month'],
                    'Total Entrances': total['Total Entrances'],
                })
...
def create_csv(parsed_args):
    ...
    write_detailed_data_to_csv(spaces, parsed_args.start_date, parsed_args.end_date, parsed_args.interval, parsed_args.tag, parsed_args.time_segment_labels)
